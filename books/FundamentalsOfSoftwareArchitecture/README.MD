...
2. What is the difference between technical portioning and domain partitioning? Provide an example of each.
    - Technical portioning is a way to organize the source code by technical capabilities, for example presentation layer, business layer, persistence layer and so on. It makes clear that all code to lead with the database is inserted in the persistence layer. The domain portioning is a way to organize the source code by workflows, where each part is bounded context of the system domain. The main difference between this portioning is the top level organization.
